...
// image icon
var imageIcon = true;
var imageUrl  = "/Tech3g_Doc_project/images/common/icon_calendar.gif";
...
function initCal(target){
    ...
    $("#" + id).click(function(e){
		...
		// image Icon
		if( imageIcon == true ){
		 	$(document.body).append($("<div id='"+id+"_calIcon' class='imageIcon'><img id='calIcon' src='"+ imageUrl +"'></div>"));
			$("#" + id + "_calIcon").css({"top":tInput.top,"left":tInput.left+tWidth});
...
function makeCal(id , tValue)
{
	 ...
	 for(var i=min_year;i<=max_year;i++){	cal_html += "<option value='"+i+"'>" + i + "年</option>"; }
	 cal_html += "</select><select id='"+id+"_calMon' style='width:60px'>";
	 for(var i=1;i<13;i++){ 	cal_html += "<option value='"+ i +"'>" + i +"月</option>"; }
	 ...
	 if( today_show == true ){
	 	 cal_html +="</tr></table></div><div align='center' id='"+id+"tBtn'><span id='"+id+"tBox'>Today</span></div></table>"
	 }else{
